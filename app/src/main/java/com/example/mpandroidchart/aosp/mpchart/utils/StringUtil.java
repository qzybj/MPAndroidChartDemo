...
public class StringUtil {
	...
	public static boolean parseBoolean(String value){
		try{
			return Boolean.parseBoolean(StringUtil.f(value));
		}catch (Exception e){
	...
	public static float parseFloat(String value){
		try{
			return Float.parseFloat(StringUtil.f(value));
		}catch (Exception e){
	...
	/**
	 * 将List转成字符串， 字段与字段之间用separator分隔
	 * @param list
	 * @param separator
	 * @return
	 */
	 ...
	 * 截取指定长度并替换(先截取，再替换字符)
	 * @param str  原字符串
	 * @param olderChar 要被替换的原来字符
	 * @param replaceChar  替换后的字符
	 * @param index 截取长度
	 * @return
	 */
	public static String getSubString(String str, String olderChar, String replaceChar, int index){
		if(StringUtil.isNotEmpty(str)&&str.length()>=index){
			String subStr = str.substring(index);
			if(StringUtil.isNotEmpty(subStr)){
				return subStr.replace(olderChar,replaceChar);
			}
		}
		return "";
	}
